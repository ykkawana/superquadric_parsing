...
def sample_gumbel(shape, eps=1e-10):
    ...
    gumble_samples_tensor = -torch.log(eps -
                                       torch.log(uniform_samples_tensor + eps))
...
def gumbel_softmax_sample(logits, tau=1, dim=None):
    ...
    soft_samples = nn.functional.softmax(gumble_trick_log_prob_samples / tau,
                                         dim)
...
def gumbel_softmax(logits, dim=None, hard=False, tau=1):
    ...
    if hard:
        ...
        samples_hard = logits.data.clone().zero_().scatter_(
            dim, max_value_indexes, 1.0)
        ...
        #   all other gradients)
        return_samples = Variable(samples_hard -
                                  samples_soft.data) + samples_soft
