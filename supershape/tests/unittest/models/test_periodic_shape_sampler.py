...
def test_transform_circumference_angle_to_super_shape_radius():
    ...
    mock_decoder = type(
        '', (nn.Module, ), {
            'forward':
            lambda self, x: torch.tensor([0.]).view(1, 1, 1).repeat(
                batch, 1, n)
        })()
    sampler.decoder = mock_decoder
    ...
    assert torch.allclose(
        target_radius.view(batch, 1, -1).repeat(1, n, 1), radius)
...
def test_transform_world_cartesian_coord_to_tsd():
    ...
    batched_points = torch.tensor([0.,
                                   0.]).view(1, 1,
                                             2).repeat(batch, points_num, 1)
    sgn = sampler.transform_world_cartesian_coord_to_tsd(xs,
                                                         ys,
                                                         preset_params,
